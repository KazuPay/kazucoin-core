...
#if defined(NDEBUG)
# error "Kazucoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Kazucoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s KAZU additional fees, %d delta bytes\n",
...
CAmount GetKAZUSubsidy(int nHeight, const Consensus::Params& consensusParams) {
	// thanks to RealSolid & WDC for helping out with this code
	CAmount qSubsidy;
    
	if (nHeight < 105000) //consensusParams.alwaysUpdateDiffChangeTarget)
	{
		qSubsidy = 100*COIN;
		//int blocks = nHeight - 0; //consensusParams.nDiffChangeTarget;
	...
	else if(nHeight< 125000) //consensusParams.workComputationChangeTarget)
	{
		qSubsidy = 50*COIN;
	...
	{
		qSubsidy = 50*COIN;
		int blocks = nHeight - 125000;
		int months = (blocks / 17472)+1;
		//decrease reward by 0.5% every month after block 125000
		for(int i = 0; i < months; i++)  qSubsidy -= (qSubsidy/200);
	}
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if(nHeight < consensusParams.nDiffChangeTarget) {
		...
		nSubsidy = GetKAZUSubsidy(nHeight, consensusParams);
	}

	//make sure the reward is at least 1 KAZU
	if(nSubsidy < COIN) {
		nSubsidy = COIN;
	}
...
void ThreadScriptCheck() {
    RenameThread("kazucoin-scriptch");
