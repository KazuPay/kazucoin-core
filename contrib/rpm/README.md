...
The RPM spec file provided here is for Kazucoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kazucoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kazucoin git repository](https://github.com/kazucoin/kazucoin) but are not part of
...
Currently only `Source10` is used, for the example `kazucoin.conf` file.
...
in the kazucoin git repository. At present only an SVG version of the Kazucoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kazucoin
...
needed when 0.12.1 is released, a proper fix is already in the Kazucoin git
master branch.
...
idea to build Kazucoin Core against a static version of that library compiled
...
The `kazucoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kazucoin-server` package. At this point, most RPM based
...
The `kazucoin-server` package should have SELinux support. How to properly do
...
Kazucoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kazucoin/). He has been
packaging Kazucoin for Fedora at least since 2012.
...
4. I renamed the `kazucoin` package that contains the Qt GUI to `kazucoin-core` as
that appears to be how the general population refers to it, in contrast to
`kazucoin-xt` or `kazucoin-classic`. I wanted to make sure the general population
