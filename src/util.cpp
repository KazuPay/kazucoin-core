...
const char * const KAZUCOIN_CONF_FILENAME = "kazucoin.conf";
const char * const KAZUCOIN_PID_FILENAME = "kazucoind.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Kazucoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Kazucoin
    // Mac: ~/Library/Application Support/Kazucoin
    // Unix: ~/.kazucoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kazucoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kazucoin";
#else
    // Unix
    return pathRet / ".kazucoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", KAZUCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", KAZUCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Kazucoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Kazucoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Kazucoin Core developers";
    }
