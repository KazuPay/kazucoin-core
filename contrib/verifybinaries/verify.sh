...
###   kazucoincore.org and kazucoin.org and compares them.
...
WORKINGDIR="/tmp/kazucoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://kazucoincore.org"
HOST2="https://kazucoin.io"
BASEDIR="/bin/"
VERSIONPREFIX="kazucoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kazucoin-',
...
if [ $? -ne 0 ]; then
   echo "kazucoin.org failed to provide signature file, but kazucoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "kazucoin.org and kazucoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Kazucoin Core binary release signing key installed?"
   fi
