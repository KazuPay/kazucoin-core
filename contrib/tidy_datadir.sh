#!/bin/bash
# Copyright (c) 2013 The Kazucoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Kazucoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Kazucoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Kazucoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Kazucoin 0.7 datadir."
    ;;
  3)
    echo "Detected Kazucoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Kazucoin 0.8 datadir."
    ;;
esac
